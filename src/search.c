...
#ifndef _GNU_SOURCE       // MKG: REDHAT PATCH 12/22/06
...
#include "system.h"
#include "grep.h"
//#include "regex.h"   // MKG: REDHAT PATCH 12/22/06
#include <regex.h>
#include "dfa.h"
#include "kwset.h"
#include "error.h"
...
# include <pcre.h>
#endif
#ifdef HAVE_LANGINFO_CODESET  // MKG: REDHAT PATCH 12/22/06
# include <langinfo.h>
#endif
...
static int using_utf8;                     // MKG: REDHAT PATCH 12/22/06
...
Gcompile (char const *pattern, size_t size)
{
  ...
  check_utf8 ();                                               // MKG: REDHAT PATCH 12/22/06
...
EGexecute (char const *buf, size_t size, size_t *match_size, int exact)
{
  ...
  // size_t i;
  size_t i; //, ret_val;        // MKG: REDHAT PATCH 12/22/06
  static int use_dfa;
  static int use_dfa_checked = 0;
#ifdef MBS_SUPPORT
//  char *mb_properties = NULL;
  const char *last_char = NULL;       // MKG: REDHAT PATCH 12/22/06
  int mb_cur_max = MB_CUR_MAX;
  mbstate_t mbs;
  memset (&mbs, '\0', sizeof (mbstate_t));
#endif /* MBS_SUPPORT */

  if (!use_dfa_checked)                              // MKG: REDHAT PATCH 12/22/06
    {
...
#else
	  use_dfa = 1;                                    // MKG: REDHAT PATCH 12/22/06
#endif /* MBS_SUPPORT */
	}
      else
	{
...
{
  register char const *beg, *end;  //*try, *end;
  register size_t len;
  char eol = eolbyte;
...
Pexecute (char const *buf, size_t size, size_t *match_size, int exact)
{
  ...
  return 0; /* quiet some compiler warnings */
}
