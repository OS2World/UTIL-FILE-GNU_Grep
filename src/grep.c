...
/*          char *buf = (char*) xmalloc (lim - beg);
      ...
      while (lim-beg && (match_offset = (*execute) (beg, lim - beg, &match_size, 1))
       ...
       fputs ("\33[K", stdout);  // MKG: REDHAT PATCH 12/22/06
    }
  fwrite (beg, 1, lim - beg, stdout);
  if (ferror (stdout))
...
#if defined(__WATCOMC__)  && (defined(__OS2__) || defined(__NT__))
  _splitpath(argv[0], NULL, NULL, prgnambuf,NULL);
  program_name = strlwr(prgnambuf);
#else
  program_name = argv[0];
...
#if (defined(__MSDOS__) || defined(_WIN32)) && !defined(__WATCOMC__)
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n"));
      ...
      printf ("\n");
#else
      printf("\n           Compiled %s with Open Watcom %d.%d\n\n",
                                           __DATE__,
                                           ((__WATCOMC__/100)- 11),
                                           (__WATCOMC__ % 100)/10);
#endif
